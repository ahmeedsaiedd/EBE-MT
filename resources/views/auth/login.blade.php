<x-guest-layout>
    <x-authentication-card>
        <x-slot name="logo">
        <img src="{{ asset('EBE_LOGO.png') }}" alt="Logo" class="w-100% h-20">
        </x-slot>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-6">
            ...
            <div class="flex items-center justify-end mt-4">
                ...
                <x-button class="ml-4 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    {{ __('Log in') }}
                </x-button>
            </div>
        </form>
